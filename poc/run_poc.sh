...
AUTO_REPLACE=1
INPUT_WAV=""
EXTERNAL_RECORDING=0

usage() {
  cat <<EOF
Usage:
  $0 [--seconds N] [--input-wav PATH] [--whisper PATH] [--model PATH] [--stt-profile fast|balanced|accurate] [--out DIR] [--audio-device NAME_OR_INDEX] [--precheck-seconds N] [--no-precheck] [--countdown-seconds N] [--no-normalize] [--stt-prompt TEXT] [--fast-ime] [--auto-paste] [--no-auto-replace] [--no-telemetry] [--telemetry-file PATH]
...
Examples:
  ...
  $0 --input-wav ./poc/.out/manual.wav --stt-profile fast
...
PADDED_WAV="${OUT_DIR}/sample_${STAMP}.padded.wav"
...
if [[ "${EXTERNAL_RECORDING}" -eq 1 ]]; then
  AUDIO_WAV="${INPUT_WAV}"
  echo "[1/5] Using external recording: ${AUDIO_WAV}"
  RECORD_LATENCY_MS=0
else
...
AUDIO_DURATION_S="$(ffprobe -v error -show_entries format=duration -of default=nk=1:nw=1 "${INPUT_FOR_STT}" 2>/dev/null || echo 0)"
if [[ "${EXTERNAL_RECORDING}" -eq 1 ]]; then
  if awk "BEGIN {exit !(${AUDIO_DURATION_S:-0} < ${MIN_EXTERNAL_STT_SECONDS})}"; then
    echo "External recording too short (${AUDIO_DURATION_S}s). Please record at least ${MIN_EXTERNAL_STT_SECONDS}s." >&2
    exit 2
  fi
fi
if awk "BEGIN {exit !(${AUDIO_DURATION_S:-0} < ${MIN_STT_SECONDS})}"; then
  echo "      Audio too short (${AUDIO_DURATION_S}s), padding to ${MIN_STT_SECONDS}s for STT stability..."
  if ffmpeg -hide_banner -loglevel error \
      -i "${INPUT_FOR_STT}" \
      -af "apad=pad_dur=${MIN_STT_SECONDS}" \
      -t "${MIN_STT_SECONDS}" \
      -ac 1 -ar 16000 -c:a pcm_s16le \
      "${PADDED_WAV}"; then
    INPUT_FOR_STT="${PADDED_WAV}"
  fi
fi
...
if ! run_stt_once "${MODEL_PATH_EFFECTIVE}"; then
  if [[ "${MODEL_PATH_EFFECTIVE}" == "${TURBO_MODEL_PATH}" && -f "${LARGE_MODEL_PATH}" ]]; then
    echo "      STT with turbo failed, fallback to large-v3..."
    MODEL_PATH_EFFECTIVE="${LARGE_MODEL_PATH}"
    run_stt_once "${MODEL_PATH_EFFECTIVE}"
  else
    echo "STT failed on model: ${MODEL_PATH_EFFECTIVE}" >&2
    exit 1
  fi
fi
STT_END_MS="$(now_ms)"
...
if [[ "${ENABLE_TELEMETRY}" -eq 1 ]]; then
  if command -v jq >/dev/null 2>&1; then
    ...
    jq -nc \
      ...
      --arg model_path "${MODEL_PATH_EFFECTIVE}" \
      ...
      --argjson external_recording "${EXTERNAL_RECORDING}" \
      ...
      '{
        ...
        model_path: $model_path,
        input_wav: $input_wav,
        external_recording: ($external_recording == 1),
